...
function showFinal(){
  ...
  const scoring=`<section class="result-guide"><strong>最终结果怎样评价</strong><p>资源只生成成长画像，不相加、不排名。教师根据行动记录和成长策略单，对每组进行0–6分的过程评价。</p><div class="score-grid"><div class="score-item"><strong>选择理由 · 0–2分</strong><span>0分未说明；1分理由基本成立；2分能联系人物处境和具体行动。</span></div><div class="score-item"><strong>识别代价 · 0–2分</strong><span>0分未看见代价；1分能说出损失；2分能说明为什么愿意承担。</span></div><div class="score-item"><strong>调整行动 · 0–2分</strong><span>0分没有调整；1分办法较笼统；2分写清删减事项、求助对象或执行时间。</span></div></div><span class="score-note">成长画像：三项核心资源差距不超过2点为“平衡成长者”；否则按最高项生成探索者、学习者或规划者，并列最高时同时显示两个画像。能量币、默契和成人沟通不额外加分，只作为成长证据。</span></section>`;
  openModal({kicker:"六周结算",title:"每个人都带着自己的方法进入考场",wide:true,body:`${scoring}${strategy}<div class="ending-grid">${endings}</div>`,actions:[{label:"查看36次行动记录",secondary:true,onClick:showJournal},{label:game.strategy?"修改成长策略单":"填写成长策略单",onClick:showStrategyForm}]});
}
